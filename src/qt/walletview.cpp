// Copyright (c) 2017 The Libercoin Core developers
    ...
    platformStyle(platformStyle)
{
    ...
    libernodePage = new Libernodes(platformStyle);
    addWidget(libernodePage);
...
void WalletView::setClientModel(ClientModel *clientModel)
{
    ...
    libernodePage->setClientModel(clientModel);
}
...
void WalletView::setWalletModel(WalletModel *walletModel)
{
    ...
    libernodePage->setWalletModel(walletModel);
...
void WalletView::gotoLibernodePage()
{
    libernodePage->statusBar->addWidget(gui->frameBlocks, 0, Qt::AlignRight);
    libernodePage->statusText->addWidget(gui->progressBarLabel);
    libernodePage->statusBar->addWidget(gui->progressBar);
    gui->menu->SimulateLibernodeClick();
    setCurrentWidget(libernodePage);
}
...
void WalletView::fetchPrice()
{
    ...
    if(configs.value("Currency").toInt() == 0)
        request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/libercoin/?convert=USD"));
    else if(configs.value("Currency").toInt() == 1)
        request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/libercoin/?convert=EUR"));
...
void WalletView::replyFinished(QNetworkReply *reply)
{
    ...
    try{
        if(stod(priceUSD) && stod(priceBTC)){
            ...
            libernodePage->priceUSD->setText(QString::fromStdString(newPriceUSD));
            libernodePage->priceBTC->setText(QString::fromStdString(priceBTC));
