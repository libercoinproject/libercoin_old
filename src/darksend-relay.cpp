...
CDarkSendRelay::CDarkSendRelay(CTxIn& vinLibernodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinLibernode = vinLibernodeIn;
    vchSig = vchSigIn;
...
std::string CDarkSendRelay::ToString()
{
    ...
    info << "vin: " << vinLibernode.ToString() <<
...
{
    CLibernode* pmn = mnodeman.GetLibernodeByRank(nRank, nBlockHeight, MIN_PRIVATESEND_PEER_PROTO_VERSION);
