...
// A helper object for signing messages from Libernodes
...
public:
     ...
     *     1) we have an active Libernode,
     *     2) we have a valid Libernode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Libernode address
    bool CheckSignature(const CPubKey& pubKeyLibernode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, libernode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 XZC - checking if valid libernode)
...
private:
    ...
    // Keep track of the used Libernodes
    std::vector<CTxIn> vecLibernodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Libernode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Libernode to start the process
    ...
    /// Get Libernode updates about the progress of mixing
...
public:
    CLibernode* pSubmittedToLibernode;
