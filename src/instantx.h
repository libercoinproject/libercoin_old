...
/*
    At 15 signatures, 1/2 of the libernode network can be owned by
...
private:
    ...
    //track libernodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapLibernodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageLibernodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointLibernode;
    std::vector<unsigned char> vchLibernodeSignature;
    // local memory only
...
public:
    CTxLockVote() :
        ...
        outpoint(),
        outpointLibernode(),
        vchLibernodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}

    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointLibernodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointLibernode(outpointLibernodeIn),
        vchLibernodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointLibernode);
        READWRITE(vchLibernodeSignature);
    }
    ...
    COutPoint GetLibernodeOutpoint() const { return outpointLibernode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapLibernodeVotes; // libernode outpoint - vote
...
public:
    ...
    bool HasLibernodeVoted(const COutPoint& outpointLibernodeIn) const;
    int CountVotes() const { return mapLibernodeVotes.size(); }
...
public:
    ...
    bool HasLibernodeVoted(const COutPoint& outpointIn, const COutPoint& outpointLibernodeIn);
